...
{
  /* 
   * Pseudocode: throughout all the pixels in the iamge 
   * for (i=0; i < 3; i++) { //where 3 can be any integer = kernel size
   *  for(j = 0; j < 3; j++) {
   *    weight += Pixel(i.j) * weight[i][j]
   */

  // the weight kernel
  const int kernel[3][3] = { {-1, 0, 1},
                             {-2, 0, 2},
                             {-1, 0, 1}};

  // alloc space for a new temp image
